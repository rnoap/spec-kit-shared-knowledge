...
- [ ] T011 Add a `--` separator to every `git` invocation in `commands/speckit.knowledge.sync.md` that interpolates a config value — `git clone … -- "$url" "$dir"`, `git sparse-checkout set -- "$p1" "$p2"` — as the second, independent defense FR-034 requires behind T005–T007 validation. **T022 and T023 rewrite these same invocations in Phase 4 and MUST preserve every separator**
...
- [ ] T023 [US2] Apply the same revision targeting to the warm path of `commands/speckit.knowledge.sync.md`, replacing the unconditional `git checkout origin/HEAD -- .` with a checkout of the declared revision when one is set, and leaving the default-revision path unchanged when none is (FR-012, FR-010). **Preserve every `--` separator added by T011** (FR-034)
